...
This example will help you integrate UNIVAULTOFFICE Docs into your web application written in .Net (C# MVC).
...
See the detailed guide to learn how to install Document Server [for Windows](https://helpcenter.univaultoffice.github.io/installation/docs-developer-install-windows.aspx), [for Linux](https://helpcenter.univaultoffice.github.io/installation/docs-developer-install-ubuntu.aspx), or [for Docker](https://helpcenter.univaultoffice.github.io/server/developer-edition/docker/docker-installation.aspx).

## Step 2. Download the .Net (C# MVC) code for the editors integration

Download the [.Net (C# MVC) example](https://api.univaultoffice.github.io/editors/demopreview) from our site.
...
```
where the **documentserver** is the name of the server with the UNIVAULTOFFICE Document Server installed and the **storage-path** is the path where files will be created and stored. You can set an absolute path.

If you want to experiment with the editor configuration, modify the [parameters](https://api.univaultoffice.github.io/editors/advanced) in the *DocEditor.aspx* file.
...
Open the *web.appsettings.config* file and [specify the same secret key](https://helpcenter.univaultoffice.github.io/installation/docs-configure-jwt.aspx) as used in your Document Server to enable JWT: 
